...
const EditServices = ({ formData, handleClick }) => {
    ...
    //     const isError = (value.match(urlReg) === null && value !== "")  ? true : false;
    //     if (isError) {
    //         setErrors(prevState => {
    //             const tempState = {...prevState};
    //             tempState[name] = "Please enter a valid url";
    //             return tempState;
    //         });
    //     } else {
    //         if (errors[name]) {
                        ...
                        return (
                            <form className={styles['edit-services-form']} onSubmit={handleSubmit}>
                                <div className={styles['form-inputs-container']}>
                                    ...
                                    >
                                        <SortableContext 
                                            items={tempServicesData.map((service) => service.id)}
                                            strategy={verticalListSortingStrategy}
                                        >
                                            {tempServicesData.map((service, i) => {

                                                    const name = service.name.split(' ').map((word, i) => {
                                                    ...
                                                    return (
                                                        <SortableItem key={service.id} id={service.id}>
                                                            ...
                                                            <Input 
                                                                ...
                                                                handleChange={(e) => handleInputChange(e, service.name, { link: e.target.value })}
                                                            />
                                                            <div className={`${styles['toggle-status']} ${service.active ? styles.active : ''}`}>{service.active ? 'Enabled' : 'Disabled'}</div>
                                                            <ToggleSwitch 
                                                                inputName={name} 
                                                                className={styles.toggle} 
                                                                name={'serviceActive'}
                                                                active={service.active}
                                                                handleToggle={(e, toggleVal) => handleInputChange(e, service.name, { active: toggleVal })}
                                                            />
                                                            {/* {errors[service.name] && <div className={styles.error}>{errors[service.name]}</div>} */}
                                                        </SortableItem>
                                                    ) 
                                                })}
                                        </SortableContext>
